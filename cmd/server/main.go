...
func main() {
	...
	if err := store.ConfigureR2(ctx, cfg); err != nil {
	...
	for i := range cfg.Collections {
		c := &cfg.Collections[i]
		if err := store.CacheExtent(ctx, c, cfg.R2Bucket); err != nil {
			log.Printf("[startup] warn: extent for %s: %v", c.ID, err)
		}
		if err := store.CacheQueryables(ctx, c, cfg.R2Bucket); err != nil {
			log.Printf("[startup] warn: queryables for %s: %v", c.ID, err)
		}
	}
	logStartup("extents and queryables cached")

	if err := store.Warmup(ctx, cfg.Collections, cfg.R2Bucket); err != nil {
